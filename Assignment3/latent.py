...
def make_ratings_matrix():
    ...
    one_fourth_data_length = int(0.25 * count)
    counter = 0
    USER_MAX = USER_MAX + 1
    MOVIE_MAX = MOVIE_MAX + 1
    ...
    with open("movies.data", "r") as data_file:
        for line in data_file:
            ...
            if (counter <= three_fourth_data_length):
                rate_train[a][b] = float(line_values[2])
            else:
                rate_test[a][b] = float(line_values[2])
    data_file.close()
...
class MF():
    ...
    def train(self):
        ...
        for i in range(self.iterations):
            ...
            # if (i+1) % 10 == 0:
            print("Count : %d ; RMSE = %.4f" % (i + 1, rmse))
            print("Count : %d ; MAE = %.4f\n" % (i + 1, mae))
        return
    ...
    def rmse(self):
        ...
        return np.sqrt(float(error) / len(xs))
